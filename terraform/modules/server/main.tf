...
# Tainted with role=observability:NoSchedule to isolate from app workloads
# ---------------------------------------------------------------------------
...
resource "hcloud_server" "observability" {
  ...
  ssh_keys           = [hcloud_ssh_key.cluster.id]
  placement_group_id = var.observability_node_count > 0 ? hcloud_placement_group.observability[0].id : null
  firewall_ids       = var.observability_firewall_id > 0 ? [var.observability_firewall_id] : [var.worker_firewall_id]
  ...
  user_data = var.cloud_init_observability_path != "" ? templatefile(var.cloud_init_observability_path, {
    k3s_version   = var.k3s_version
    k3s_token     = local.k3s_token
    api_server_lb = var.api_server_lb_ip
    ...
    cluster_name  = var.cluster_name
    network_cidr  = var.network_cidr
  }) : templatefile(var.cloud_init_worker_path, {
    k3s_version   = var.k3s_version
    k3s_token     = local.k3s_token
    api_server_lb = var.api_server_lb_ip
    node_name     = "${var.cluster_name}-obs-${count.index}"
